...
from mezzanine_mises.mises import models
...
def random_slogan():
    ...
    return models.Slogan.objects.all().order_by('?').values('slogan')[0]['slogan']
...
def blog_post_list(context):
    ...
    posts = blog_models.BlogPost.objects.filter(publish_at__lte=now).select_related().order_by('-publish_at')
